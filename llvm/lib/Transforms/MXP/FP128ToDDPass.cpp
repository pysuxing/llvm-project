...
#define CF2(I, s)                                                              \
  ChangeInstToCall(I, s, {I->getOperand(0), I->getOperand(1)}, I->getType())
#define CF3(I, s)                                                              \
  ChangeInstToCall(I, s,                                                       \
                   {I->getOperand(0), I->getOperand(1), I->getOperand(2)},     \
                   I->getType())
#define CB2(I, s)                                                              \
  ChangeInstToCall(I, s, {I->getOperand(0), I->getOperand(1)},                 \
                   Type::getInt1Ty(M.getContext()))
#define CT1(I, s) ChangeInstToCall(I, s, {I->getOperand(0)}, I->getType())

template <typename T>
inline void FP128ToDDPass::ChangeInstToCall(T *I,
  ...
  if (!(dyn_cast<StoreInst>(I))) {
    for (unsigned i = 0; i < Args.size(); i++) {
      ...
      if (constant &&
          arg->getType()->isPPC_FP128Ty()) { //如果函数参数中有fp128常量，先转成dd
        Function *FP128ToDD = M->getFunction("FP128ToDD");
        if (!FP128ToDD) {
          Type *fp128Type = Type::getFP128Ty(M->getContext());
          FunctionType *funType =
              FunctionType::get(Type::getVoidTy(M->getContext()),
                                {fp128Type, fp128Type->getPointerTo()}, false);
          FP128ToDD = Function::Create(funType, Function::ExternalLinkage,
                                       "FP128ToDD", M);
        }
  ...
  if (!Callee || Callee->getFunctionType() != FuncTy) {
    Callee =
        Function::Create(FuncTy, Function::ExternalLinkage, FunctionName, M);
  }
...
//     CallInst *Call = Builder.CreateCall(Func, {I->getOperand(0),
//     I->getOperand(1)}); Call->setDebugLoc(I->getDebugLoc());
//
    ...
    for (auto &BB : F) {
      for (auto &I : BB) {
        /*if (auto *SI = dyn_cast<StoreInst>(&I)) {
          if (SI->getValueOperand()->getType()->isPPC_FP128Ty()) {
            ...
            ////此处可能存在精度损失
            // doubleVal.convert(APFloat::IEEEdouble(),
          ...
          if (BinOp->getType()->isPPC_FP128Ty() &&
              !funName.empty()) { //处理fp128的add,sub,mul,div
            for (unsigned i = 0; i < BinOp->getNumOperands(); ++i) {
              if (BinOp->getOperand(i)->getType()->isPPC_FP128Ty()) {
                funName += "2";
          ...
          Instruction *II = &I;
          if (II->getType()->isPPC_FP128Ty()) {
            CF1(II, "ppc_neg");
          }
        } else if (auto *FCmp = dyn_cast<FCmpInst>(&I)) { // 处理比较指令
          if (FCmp->getOperand(0)->getType()->isPPC_FP128Ty()) {
          ...
          if (Callee) {
            if (Callee->getReturnType()->isPPC_FP128Ty()) {
              ////errs() << "FP128 Call : " << I << "\n";
              if (Callee->getName() == "llvm.fmuladd.ppcf128") {
#ifdef DEBUG
                // errs() << "This call (" << I << ") need to deal." << "\n";
#endif
                std::string funName = "ppc_muladd";
                for (unsigned i = 0; i < FCall->getNumOperands(); ++i) {
                  if (FCall->getOperand(i)->getType()->isPPC_FP128Ty()) {
                    funName += "2";
            ...
            if (Callee->getName() == "printf" &&
              ...
              for (unsigned i = 0; i < FCall->getNumOperands(); ++i) {
                ...
                if (pa->getType()->isPPC_FP128Ty()) {
...
#else
                  Function *Fun = M.getFunction("ppc_printf");
                  if (!Fun) {
                    Fun = Function::Create(
                        FunctionType::get(Type::getInt32Ty(M.getContext()),
                                          {pa->getType()}, false),
                        Function::ExternalLinkage, "ppc_printf", &M);
                  }
                  ...
                  // ChangeInstToCall(FCall, "DDToFP128", {I->getOperand(0)},
                  // I->getType()); if (auto *ap = dyn_cast<Instruction>(pa)) {
                  //	//errs() << "ap : " << *ap << "\n";
              ...
              // if (hasFP128Pa) {
              //	//errs() << "This printf : " << *FCall << " need to
              //deal.\n";
              //
              // }
            }
...
//            if (SrcType->isPPC_FP128Ty() && DstType->isDoubleTy()) {
//              CT1(CI, "DDToDouble");
//            } else if (SrcType->isPPC_FP128Ty() && DstType->isFloatTy()) {
//              CT1(CI, "DDToFloat");
...
//            if (SrcType->isDoubleTy() && DstType->isPPC_FP128Ty()) {
//              CT1(CI, "DoubleToDD");
//            } else if (SrcType->isFloatTy() && DstType->isPPC_FP128Ty()) {
//              CT1(CI, "FloatToDD");
...
//            if (SrcType->isPPC_FP128Ty() && DstType->isIntegerTy()) {
...
//            if (SrcType->isIntegerTy() && DstType->isPPC_FP128Ty()) {
